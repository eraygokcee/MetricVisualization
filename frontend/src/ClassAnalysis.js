...
export default function ClassAnalysis({ data , onBack}) {
  ...
  const rows = results.map(({ id, className, tcc, wmc, lcom, dit, cbo, maxCyclo, avgCyclo }) => ({
    id, className, tcc, wmc, lcom, dit, cbo, maxCyclo, avgCyclo
  }));
  const columns = [
